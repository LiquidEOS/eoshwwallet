...
let {ecc, Fcbuffer, fc} = Eos.modules;
...
class InputMessage {
	...
	drawOptions(){				
		...
		for(var i=this.currentLine * 9; (i < choices.length) && (i < (this.currentLine+1) * 9); i++){
			var j = i % 9;
			drawBox(3+ j * 13,18, 10 ,16,true, i === this.selection);
			drawText(3+j*13+1,19,choices[i].toString(), i !== this.selection);
		}
...
app.get('/', async (req, res) => {
	...
	res.send(JSON.stringify({key:publicKey}));
});
